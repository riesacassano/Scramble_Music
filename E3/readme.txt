...
`alignment_original.csv` contains alignment values. The output of `E3_compute_precision_sensitivity.ipynb` will be called `alignment.csv`. Note that because samples are randomly generated for the null distribution, there may be some variability in alignment values. Thus we include the "original" alignment values since they are what we report in the paper.


Notebooks and scripts:

`E3_rate.Rmd` takes `response_rate_by_sub.csv`. It runs non-parametric ANOVA and generates figure 3.

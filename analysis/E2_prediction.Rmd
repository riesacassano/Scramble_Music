...
library(logspline) # version 2.1.22
```
...
```{r}
data <- read_csv('../data/E1-E2-E4/prediction.csv', show_col_types = FALSE)
```
...
```{r}
print(summary(mus_scram, robust = TRUE), digits = 4)
```
...
```{r}
print(summary(mus_only, robust = TRUE), digits = 4)
```
...
```{r}
print(summary(mus_scram_int, robust = TRUE), digits = 4)
```
...
```{r}
plot(only1B)
print(summary(only1B, robust = TRUE), digits = 4)
```
...
```{r}
plot(no1B)
print(summary(no1B, robust = TRUE), digits = 4)
```
...
no1B_int <- brm(response ~ Musician*scramble + (1 | exp_subject_id), data = data_no1B,
                ...
                prior = c(
                  ...
                  set_prior('normal(0, 1)', coef = 'Musician1:scramble2B')
                  ),
                save_pars = save_pars(all = TRUE), iter = 20000, refresh = 0,
...
```{r}
plot(no1B_int)
print(summary(no1B_int, robust = TRUE), digits = 4)
```
...
print(summary(years_mus_scram), robust = TRUE, digits = 5)
```
...
```{r}
plot(years_mus)
print(summary(years_mus, robust = TRUE), digits = 4)
```
